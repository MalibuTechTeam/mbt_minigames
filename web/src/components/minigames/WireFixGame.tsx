...
const WireFixGame: React.FC = () => {
  ...
  }, [cols, rows, startRow, endRow, status, handleEnd]);
  ...
  useEffect(() => {
    ...
    while (currentCol < cols - 1 || currentRow !== eRow) {
      ...
      if (currentCol === cols - 1) moveDir = "VERT";
      else if (currentRow !== eRow && Math.random() > 0.5) moveDir = "VERT";

      if (moveDir === "RIGHT") currentCol++;
      else {
  ...
  }, [grid.length, status, updatePowerState]);
